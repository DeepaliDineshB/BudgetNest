...
def addmember(request):
    if request.method == 'POST':
        ...
        new_member = FamilyData.objects.create(name=name,age=age,income=income,family_lead=request.user)
...
def update_data(request,id):
    family_members=FamilyData.objects.get(id=id)
    if request.method == 'POST':
        family_members.name = request.POST.get('firstname')
        family_members.age = request.POST.get('age')
        family_members.income = request.POST.get('income')
        family_members.save()
        return redirect('familydata')
    return render(request,'update_data.html',{'data':family_members})
...
def addexpenses(request):
    family_members=FamilyData.objects.filter(family_lead=request.user)
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request,'addexpenses.html',{'data':family_members,'error':'Invalid family member selected'})
    ...
    return render(request,'addexpenses.html',{'data':family_members})
